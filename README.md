...
| `deduplicate_csv.py` | **Optional** post-process: redirect duplicate large (>10 MiB) assets to a single master copy via the additive `master_png_path` column (feature 006), and renumber within-week gram-number collisions via the additive `target_gram_id` column (feature 007). |
...
5. **Stage 5 — DITA generation.** `generate_dita.py` consumes the
   ...
   For `main`, the chapter is the **week** (feature 007): a bare-integer
