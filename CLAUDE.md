...
Two things are decided at **build time, by transformation-scenario parameter**,
not inferred from page content:

- **The protective marking** (`webhelp.show.protection`,
  `webhelp.protection.text`, `webhelp.protection.background.color`) — a bar top
  and bottom of every page, the same text and colour in both editions.
- **Search-box visibility** (`webhelp.show.search`) — `yes` in the template
  (instructor); the student scenario alone sets `no`.
...
per parameter. The fragments bind to `webhelp.fragment.before.body` /
`after.body` / `after.header` — the placeholders that reach **all four** page
types — and all four page-type XSLT extension points import both includes.
...
- **Anything that might change per export belongs in a scenario parameter, not
  ...
  scenario parameter is a local edit on the target.
