...
`theme/oxygen-hide-search/`, `theme/search-in-header/`,
`theme/oxygen-dark-mode/`, `theme/oxygen-protection/`,
...
The fragments bind to `webhelp.fragment.before.body` / `after.body` /
`after.header` — the placeholders that reach **all four** page types — and all
four page-type XSLT extension points import both includes. `oxyf:getParameter`
can read DITA-OT's own properties, not just the template's, because Oxygen's
`whr-create-props-file` serialises *every* Ant property of the build.

Three rules follow, all learned the hard way:

- **Never infer the edition from page content.** The `.gf-persistent` marker
  `generate_dita.py` stamps exists only on pages built from a **topic**.
  `index.html` is built from the ditamap and `search.html` is generated by
  Oxygen, so neither carries it in *either* edition — which is exactly how the
  instructor lost the search box on the tiles landing page (#178).
- **Don't add a scenario parameter whose absence fails silently.** Operators
  rebuild scenarios from the stock built-in; anything they must remember to
  re-add, they will eventually forget. A `webhelp.show.search` parameter was
  tried and rejected on exactly this ground — forgetting it yields a student
  edition with a dead search box and no complaint, whereas forgetting
...
- **Anything that might change per export belongs in a scenario parameter, not
  ...
  scenario parameter is a local edit on the target. The two rules above bound
  this one — it is an argument for the *marking* being a parameter, not a
  licence to make every behaviour one.

Do **not** fork the stock `page-templates/wt_*.html` layouts to host new
placeholders (Fi3ldMan does; it means re-diffing them against stock on every
Oxygen upgrade). The fragment points above cover every page type without a fork.

The WebHelp CLI needs a licence key this project does not have, so **the
publish cannot be verified locally** — it is a manual Oxygen run. The
cross-file wiring guard in `test_package_release.py` is what stands between an
edit and that run.
