...
| 007 | Realign pipeline with the audited Lofar → `.glc` → `.png`/`.jpg`/`.wav` model — the source docs originally said "Lofar links point to `.glc` files, a small number may point to `.wav` instead", and the pipeline was built around that. The audited corpus (1,004/1,004 `Lofar` text-run hyperlinks) shows **every** Lofar link targets a `.glc`; the 18% `.wav` case is always one indirection deeper, inside the `.glc`'s `data_source/filename`. **Spec contract now defined** (see `high-level-spec.md` §1.5/§6, `spec.md` FR-011, `csv-schema.md` columns 13/15, `glc-schema.md` "Asset extension contract", `dita-topic-schema.md` §1/§1.2/§1.3/§2): the generator dispatches purely on the inner asset's extension — `.png`/`.jpg` → embed inline (§1.2 GramFrame table); `.wav` → `<xref>` to the `.glc` with both `.glc` and `.wav` copied alongside the topic (§1.3 GLC-viewer link); anything else → skipped. `wav_treatment` is retired (deprecated, ignored). Consequential code follow-up: (a) `extract_to_csv.py:335` filter `endswith((".glc", ".wav"))` → tighten to `.glc` only and warn loudly on anything else; (b) `extract_to_csv.py:432-441` `is_wav` branch + `"WAV link; treatment required"` warning is dead — remove; (c) `generate_dita.py:emit_wav_stub_topic` and `dispatch_row`'s `is_wav_link` branch trigger on `link_href.endswith(".wav")` — replace with extension-on-`png_path` dispatch matching the new §1.3 contract (copy `.glc` + `.wav` pair, emit `<xref>` to the `.glc`, no `<image>`); (d) bug currently emits `<image href="*.wav">` for these rows producing broken `<img>` icons in published HTML — fixed automatically once (c) lands; (e) `test_extract_to_csv.test_wav_link_row_shape` and `test_generate_dita.test_wav_gaps_lite_stub` rewrite for the GLC-inner-WAV path; (f) `mock_pptx.py`'s `_emit_wav_link_grams` (grams 5 and 20) generates Lofar text runs that hyperlink directly to `.wav` — replace with the realistic case (Lofar→`.glc`, `.glc`→`.wav`). Capturing as one item because they share a single root cause and should be touched together. | Tech debt | — | 4 | 2 | 4 | 10 | Medium | clarified |
