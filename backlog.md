...
| 007 | Realign pipeline with the audited Lofar → `.glc` → `.png`/`.wav` model — the source docs originally said "Lofar links point to `.glc` files, a small number may point to `.wav` instead", and the pipeline was built around that. The audited corpus (1,004/1,004 `Lofar` text-run hyperlinks) shows **every** Lofar link targets a `.glc`; the 18% `.wav` case is always one indirection deeper, inside the `.glc`'s `data_source/filename`. Source docs corrected in this PR (README.md, high-level-spec.md §1.5, csv-schema.md columns 9/10/15 + rule 4 + worked example, gramframe.md §5). Consequential follow-up across the toolchain: (a) `extract_to_csv.py:335` filter `endswith((".glc", ".wav"))` → tighten to `.glc` only and warn loudly on anything else; (b) `extract_to_csv.py:432-441` `is_wav` branch + `"WAV link; treatment required"` warning is dead in real data — remove or convert into an anomaly-detection assertion; (c) `generate_dita.py:emit_wav_stub_topic` and `dispatch_row`'s `is_wav_link` branch trigger on `link_href.endswith(".wav")` — rewire to trigger on `png_path.endswith(".wav")` so the `wav_treatment` enum applies to the GLC-inner-WAV case it was always meant for; (d) the DITA `<image href>` currently emits the raw `.wav` filename for GLC-inner-WAV rows when `wav_treatment` is empty, producing broken `<img>` tags in published HTML (visible in the SME deck's gram_04 sample) — block emission unless `wav_treatment` is decided, or surface the row in `skipped.txt`; (e) `test_extract_to_csv.test_wav_link_row_shape` and `test_generate_dita.test_wav_gaps_lite_stub` assert the phantom hyperlink-level WAV contract — rewrite to assert the GLC-inner-WAV path; (f) `mock_pptx.py`'s `_emit_wav_link_grams` (grams 5 and 20) generates Lofar text runs that hyperlink directly to `.wav` — replace with the realistic case (Lofar→`.glc`, `.glc`→`.wav`). Capturing as one item because they share a single root cause and should be touched together. | Tech debt | — | 4 | 2 | 4 | 10 | Medium | proposed |
