...
<!--
    Customizations for the INDEX TERMS page extension point,
    com.oxygenxml.webhelp.xsl.createIndexTermsPage. Oxygen only emits this
    page when the publication has indexterms - ours has none today - but it
    is wired anyway, so the page cannot appear unmarked if one is ever added.
