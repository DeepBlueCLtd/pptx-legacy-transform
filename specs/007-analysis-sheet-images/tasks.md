...
(`tests/test_snapshot_analysis_docs.py`), extensions to
...
- [X] T001 Create `snapshot_analysis_docs.py` at repo root following the
  `verb_noun.py` / `main(argv) -> int` + `argparse` + `setup_logging(Path("snapshot.log"))`
  ...
  `specs/007-analysis-sheet-images/contracts/snapshot-cli.md`; leave the scan/render
...
**Checkpoint**: `python snapshot_analysis_docs.py --content-root <empty-dir>`
runs, writes `snapshot.log`, exits 0.
...
- [X] T003 In `snapshot_analysis_docs.py`, implement
  ...
  research R7).
- [X] T004 In `snapshot_analysis_docs.py`, implement
  `needs_render(doc: Path) -> bool` returning `True` iff no same-stem `.png`
  sibling exists (`doc.with_suffix(".png")`), and a `SnapshotResult` dataclass
...
**Independent Test**: Run `snapshot_analysis_docs.py` (renderer = the T002
...
- [X] T005 [P] [US1] In `tests/test_snapshot_analysis_docs.py` add
...
- [X] T006 [P] [US2] In `tests/test_snapshot_analysis_docs.py` add
  `test_docx_only_folder_produces_png`: as T005 but `bbb_analysis.docx`.
- [X] T007 [P] [US1] In `tests/test_snapshot_analysis_docs.py` add
...
- [X] T008 [P] [US2] In `tests/test_snapshot_analysis_docs.py` add
...
- [X] T010 [US1] In `snapshot_analysis_docs.py`, implement
...
- [X] T011 [US1] In `snapshot_analysis_docs.py`, implement the `main()` scan
  ...
  nothing). Accumulate `SnapshotResult`s. This makes T005–T008 pass.
...
- [X] T014 [US1] Update `run_pipeline.bat`: insert
  `python snapshot_analysis_docs.py --content-root %1` as a new first stage
  before the extract step, with `if errorlevel 1 goto error`. New order:
  snapshot → extract → pause → generate. (FR-006)
...
WARNINGs that defer (run continues, exit 0) and are surfaced in `snapshot.log`,
...
- [X] T015 [P] [US3] In `tests/test_snapshot_analysis_docs.py` add
...
- [X] T016 [P] [US3] In `tests/test_snapshot_analysis_docs.py` add
...
- [X] T018 [US3] In `snapshot_analysis_docs.py`, make `render_doc_to_png`
  ...
  raises. (FR-008)
- [X] T019 [US3] In `snapshot_analysis_docs.py`, implement
...
- [X] T020 [US3] In `snapshot_analysis_docs.py`, implement the end-of-run
...
- [X] T022 [P] In `tests/test_snapshot_analysis_docs.py` add
...
- [X] T023 [P] In `tests/test_snapshot_analysis_docs.py` add
...
- [X] T024 In `snapshot_analysis_docs.py`, implement `tidy_image(png: Path) ->
...
- [X] T025 In `snapshot_analysis_docs.py`, call `tidy_image` in `main` right
...
- [X] T026 [P] In `tests/test_snapshot_analysis_docs.py` add
  ...
  `docx_wrapped=True`.
- [X] T027 [P] In `tests/test_snapshot_analysis_docs.py` add
...
- [X] T028 In `snapshot_analysis_docs.py`, implement `wrap_png_in_docx(png:
...
- [X] T029 In `snapshot_analysis_docs.py` `main`, for an analysis sheet that
...
- [X] T030 [P] Update `tests/test_run_pipeline_bat.py`
  ...
  `snapshot → extract → pause → generate` (a `snapshot_analysis_docs.py`
  index before `extract_to_csv.py`) and an `errorlevel` guard on the snapshot
...
- [X] T033 [P] In `specs/001-pptx-dita-migration/contracts/csv-schema.md`, clarify
  ...
  `png_path` carries the rendered sibling `.png` from `snapshot_analysis_docs.py`
...
- [X] T034 [P] In `specs/001-pptx-dita-migration/contracts/cli-contracts.md`, add a
  `snapshot_analysis_docs.py` stanza mirroring
...
- [X] T036 Execute `specs/007-analysis-sheet-images/quickstart.md` end-to-end
  ...
  byte-identical re-run, failures visible) and idempotency (second snapshot run
  writes nothing).
...
- **Determinism**: rendered PNG and reverse `.docx` are committed source assets;
  the reverse wrap uses a fixed `date_time`; the snapshotter is a no-op on an
